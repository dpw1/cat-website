...
exports.sourceNodes = async (
  ...
  try {
    ...
    console.error(
      'ez-woocommerce-source: Error initialising WooCommerce REST API. Error: ',
      err
    );
    return;
  }
  ...
  async function fetchNodesAndCreate(array) {
    ...
    for (const field of array) {
      ...
      if (limit !== null) {
        tempNodes = tempNodes.slice(0, limit);
      }

      if (verbose) {
        timeStampedLog(
          `ez-woocommerce-source: Fetching ${tempNodes.length} nodes for field: ${field}`
        );
      }
      ...
      if (verbose) {
        timeStampedLog(
          `ez-woocommerce-source: Completed fetching nodes for field: ${field}`
        );
      }
    }
    ...
    if (verbose) {
      ...
      console.log(
        `ez-woocommerce-source: skipping "mapRelatedProducts" function.`
      );
    }
